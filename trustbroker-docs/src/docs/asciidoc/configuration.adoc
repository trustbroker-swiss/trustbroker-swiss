...
- A global model can be defined in the trustbroker.config.qoa section so the SetupRP/SetupCP Qoa sections do not need to define
the order number (specifying which ACClass is stronger than another).
- SetupRP defines defaults an RP requires (assuming default comparison type exact). This list is used when the RP does not send
any requirements or it's used to validate incoming requirements.
- SetupCP defines what the federated IdP supports and is used for outbound mapping (using the internal order numbers) and also
to validate, what the CP returns.
