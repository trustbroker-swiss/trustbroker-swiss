...
class SamlOutputServiceTest extends ServiceSamlTestUtil {
	...
	private final static String ENDPOINT_ACTION = " action=\"" + HTMLEncoder.encodeForHTMLAttribute(ENDPOINT) + '"';

	private static final DestinationType DEST_ALIAS = DestinationType.RP;
	...
	void sendPostRequest() throws Exception {
		...
		samlOutputService.sendRequest(request, credential, RELAY_STATE, ENDPOINT, httpResponse, encodingParams, DEST_ALIAS);
	...
	void sendPostResponse() throws Exception {
		...
		samlOutputService.sendResponse(response, null, RELAY_STATE, ENDPOINT, httpResponse, encodingParams, DEST_ALIAS);
