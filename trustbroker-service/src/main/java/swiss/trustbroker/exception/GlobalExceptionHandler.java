 ...
 * are either thrown when data from the network is unexecpted (in this case we can track back to code without the
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<String> handleDeniedException(RequestDeniedException ex) {
		logException(ex, ex.getInternalMessage(), log.isDebugEnabled());
		return redirectToErrorPage(ex);
	}

	@ExceptionHandler(TechnicalException.class)
	public ResponseEntity<String> handleTechnicalException(TechnicalException ex) {
		logException(ex, ex.getInternalMessage(), log.isErrorEnabled());
		return redirectToErrorPage(ex);
	}
	...
	private String getErrorPageUrl(ErrorCode errorCode) {
		return apiSupport.getErrorPageUrl(errorCode.getLabel(), WebSupport.getTraceId());
	}

	private void logException(Exception ex, String internalMessage, boolean withStack) {
		logException(ex, internalMessage, withStack, trustBrokerProperties.getNetwork());
	}
	...
	public static int logException(Throwable ex, NetworkConfig networkConfig) {
		if (ex instanceof RequestDeniedException rex) {
			...
			logException(tex, tex.getInternalMessage(), true, networkConfig);
		}
			...
			logException(eex, "-", true, networkConfig);
		}
		else {
	...
	public static Exception getRootCause(Exception ex) {
		var causingEx = ex;
		while (causingEx.getCause() instanceof Exception cex) {
			causingEx = cex;
		}
		return causingEx;
	}
	...
	public static String getMessageOfExceptionOrCause(Exception ex, boolean useToStringForCause) {
		...
		var causingEx = GlobalExceptionHandler.getRootCause(ex);
